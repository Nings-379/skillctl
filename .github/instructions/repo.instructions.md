...
```bash
# 添加仓库 (自动生成名称)
skillctl add -r https://github.com/Nings-379/ninskills

# 添加仓库并指定名称
skillctl add -r https://skillsllm.com/api/skills --repo-name official

# 添加仓库并设置为默认
skillctl add -r https://github.com/user/repo --repo-default
...
```bash
skillctl remove --repo ninskills
```

### 4. 设置默认仓库

```bash
skillctl install default-repo <name>
```
...
```bash
skillctl install default-repo official
```

### 5. 索引/同步仓库

```bash
skillctl install sync-repo [name]
```
...
# 同步指定仓库
skillctl install sync-repo official

# 同步另一个仓库
skillctl install sync-repo ninskills
```
...
```
github.com/seekthought/skill/
├── cmd/
│   ├── add/
│   │   └── index.go         # add 主命令（含 --repo 逻辑）
│   ├── install/
│   │   ├── index.go         # install 主命令
│   │   └── repository.go    # sync-repo/default-repo 子命令
│   ├── list/
│   │   └── index.go         # list 主命令（含 --repo 逻辑）
│   └── remove/
│       └── index.go         # remove 主命令（含 --repo 逻辑）
├── pkg/
│   └── db/
...
当前的远程仓库管理命令提供了完整的远程仓库管理解决方案:

- `skillctl add -r <url>`: 添加仓库
- `skillctl list --repo`: 查看所有仓库
- `skillctl install default-repo <name>`: 设置默认仓库
- `skillctl install sync-repo [name]`: 同步仓库索引
- `skillctl remove --repo <name>`: 删除仓库
