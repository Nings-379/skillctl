...
- ✅ 列出本地、项目或远程技能
- ✅ 添加技能到项目
- ✅ 搜索技能（支持本地、项目目录、GitHub 全局搜索）
- ✅ 移除技能（支持本地和项目）
- ✅ 显示技能详细信息（包括安装时间）
- ✅ 支持多种技能目录结构
- ✅ 版本管理（推送时自动打标签、安装指定版本、列出可用版本）
...
- `push` - 上传技能到远程仓库
- `remove` - 从本地或项目目录移除技能
- `search` - 在本地目录、项目目录、仓库索引或 GitHub 中搜索技能
- `status` - 列出所有已安装的技能
...
```cmd
git clone https://github.com/seektought/ai-agent-skill.git
cd .\ai-agent-skill
...
```bash
skillctl install default-repo <仓库名称>
```
...
# 同步指定仓库
skillctl install sync-repo <仓库名称>
```
...
从已索引的数据库中安装技能。需要先使用 `skillctl install sync-repo` 同步仓库。

**安装流程：**

1. 检查技能是否已安装
2. 在数据库中搜索技能（支持精确匹配和模糊匹配）
3. 如果使用 `-r <repo-name>`，只在指定仓库中搜索
...
使用格式为 `<skill-name>@<version>` 的 git 标签来检出指定版本。标签在推送带有 `version` 字段的 SKILL.md 时自动创建。
...
通过查询远程仓库中格式为 `<skill-name>@<version>` 的 git 标签来列出所有可用版本。
...
1. 在数据库中搜索技能（支持通过 `-r <repo-name>` 指定仓库）
2. 显示当前版本信息
...
使用 GitHub Search API 搜索带有 `skills/` 目录的仓库，并匹配其中的技能目录名称，输出：
...
没有 `skills/` 目录的仓库会被忽略。如果设置了 `GITHUB_TOKEN`，可以获得更稳定的速率限制。
...
使用 GitHub/GitLab API 上传技能到指定的远程仓库。

**自动版本标签：**

推送到远程仓库时，如果技能的 SKILL.md 包含 `version` 字段，skillctl 会自动：

1. 创建格式为 `<skill-name>@<version>` 的 git 标签（如 `pdf@1.2.0`）
2. 将标签推送到远程仓库
3. 如果相同版本的标签已存在，会强制更新

这使得可以通过 `skillctl install <skill-name> -v <version>` 安装指定版本。

```bash
# 示例：推送 SKILL.md 中版本为 1.2.0 的技能
skillctl push pdf -r my-repo
# 输出：
...
# 从 examples/skills 通过 URL 推送到远程
skillctl push pdf -R https://github.com/example/skills

# 从 examples/skills 通过已登记仓库名推送到远程
skillctl push pdf -r my-repo
```
...
列出本机记录的 skill 安装历史，包括技能名称、来源、下载人和下载时间。
...
需要远程仓库中存在版本标签。标签在使用 `skillctl push` 推送时自动创建。
...
A: 当推送包含 `version` 字段的 SKILL.md 时，skillctl 会自动创建 `<skill-name>@<version>` 格式的 git 标签。每个版本号对应一个独立的标签。更新版本号并重新推送会创建新标签，旧标签保留。
