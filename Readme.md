...
- ✅ Search skills (supports local, project directories, and GitHub global search)
...
- ✅ Version management (auto-tagging on push, install specific versions, list available versions)
...
- `search` - Search for skills in local directories, project directories, repository index, or GitHub
...
- `-g, --global` - Search GitHub globally for skill names inside `skills/` directories
...
No Go environment required. Downloads a pre-built binary and installs it.

**Linux/macOS:**

```bash
curl -fsSL https://raw.githubusercontent.com/Nings-379/skillctl/master/install.sh | bash
```

**Windows (PowerShell):**

```powershell
irm https://raw.githubusercontent.com/Nings-379/skillctl/master/install.ps1 | iex
```
...
skillctl add -r https://github.com/user/repo

# Add repository and specify name
skillctl add -r https://github.com/user/repo --repo-name my-repo

# Add repository and set as default
skillctl add -r https://github.com/user/repo --repo-name my-repo --repo-default

# Add repository and set priority
skillctl add -r https://github.com/user/repo --repo-priority 10
```
...
```bash
skillctl install default-repo <repository-name>
```

Example:

```bash
skillctl install default-repo my-repo
```

#### Remove Repository

```bash
skillctl remove --repo <repository-name>
```
...
skillctl install sync-repo

# Sync specific repository
skillctl install sync-repo <repository-name>
```
...
# You can also pass the URL directly as a positional argument
...
Both remote listing styles are supported:
...
#### List Available Versions of a Skill

```bash
# List all versions of a skill from a specific repository
skillctl list --versions --repo=<repo-name> <skill-name>

# List versions using database lookup
skillctl list --versions <skill-name>
```

Lists all available version tags for a skill by querying remote git tags in the format `<skill-name>@<version>`.
...
Install skills from the indexed database. Requires using `skillctl install sync-repo` to sync repositories first.

**Installation Process:**

1. Check if skill is already installed
2. Search for skill in database (supports exact match and fuzzy match)
3. If `-r <repo-name>` is used, search only in the specified repository
...
skillctl install <skill-name> -v <version>

# Install from specific repository with version
skillctl install -r <repo-name> <skill-name> -v <version>

# Examples
skillctl install pdf -v 1.0.0
skillctl install -r my-repo unit-test-generator -v 2.1.0
```

Uses git tags in the format `<skill-name>@<version>` to checkout the exact version. Tags are automatically created when pushing skills with a `version` field in SKILL.md.
...
**Search GitHub globally for skill names inside `skills/` directories:**

```bash
skillctl search --global pdf
skillctl search -g prompt --limit 5
```

This uses the GitHub Search API to look for skill directory names inside repositories that contain a `skills/` folder and prints:
...
Repositories without a `skills/` folder are ignored. If `GITHUB_TOKEN` is set, the command gets better rate-limit headroom.
...
```bash
skillctl push pdf --remote-url https://github.com/user/repo
```

Or use short flags:

```bash
skillctl push pdf -R https://github.com/user/repo
```

**Push by registered repository name:**

```bash
skillctl push pdf --remote-repo my-repo

# Short flag
skillctl push pdf -r my-repo
```

Upload skills to the specified remote repository using GitHub or GitLab API.

**Automatic Version Tagging:**

When pushing to a remote repository, if the skill's SKILL.md contains a `version` field, skillctl automatically:

1. Creates a git tag in the format `<skill-name>@<version>` (e.g. `pdf@1.2.0`)
2. Pushes the tag to the remote repository
3. If the same version tag already exists, it will be force-updated

This enables version-specific installation via `skillctl install <skill-name> -v <version>`.

```bash
# Example: push a skill with version 1.2.0 in SKILL.md
skillctl push pdf -r my-repo
# Output:
#   🏷️  Creating version tag: pdf@1.2.0
#   ✓ Tag created
#   ✓ Tag pushed
```
...
# Push from examples/skills to remote by URL
skillctl push pdf -R https://github.com/example/skills

# Push from examples/skills to remote by registered repository name
skillctl push pdf -r my-repo
```
...
```bash
skillctl status downloads

# View history for a specific skill
skillctl status downloads pdf
```

List locally recorded skill install history, including skill name, source, downloader, and download time.
...
skillctl push my-skill -R https://github.com/user/repo
...
# Push to remote repository by URL
skillctl push pdf -R https://github.com/user/repo

# Push to remote repository by registered repository name
skillctl push pdf -r my-repo
```
...
### Q: How to install a specific version of a skill?

A: Use the `--version` or `-v` flag:

```bash
skillctl install pdf -v 1.0.0
skillctl install -r my-repo pdf -v 1.0.0
```

This requires version tags to exist in the remote repository. Tags are automatically created when pushing with `skillctl push`.

### Q: How to see all available versions of a skill?

A: Use `--versions` flag:

```bash
skillctl list --versions --repo=my-repo pdf
```

### Q: How does version tagging work?

A: When you push a skill with a `version` field in SKILL.md, skillctl automatically creates a git tag `<skill-name>@<version>`. Each version gets its own tag. Updating the version number and pushing again creates a new tag while keeping old ones.
