...
// RecordSkillDownload stores a local install/download event for later audit and analysis.
func (m *Manager) RecordSkillDownload(ctx context.Context, arg CreateSkillDownloadParams) error {
	if arg.DownloadedAt == "" {
		arg.DownloadedAt = time.Now().Format(time.RFC3339)
	}
	return m.q.CreateSkillDownload(ctx, arg)
}
